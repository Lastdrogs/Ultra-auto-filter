...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("👋 Hᴇʏ Sᴇᴀʀᴄʜ 🔍 Yᴏᴜʀ Oᴡɴ...!", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton("Nᴇxᴛ ⪼", callback_data='helpsk')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            ...
            InlineKeyboardButton('《 SᴛᴀᴛᴜS 》', callback_data='stats'),
            InlineKeyboardButton('Nᴇxᴛ ⪼', callback_data='helpkr')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.HELP_TXT.format(query.from_user.mention),
        ...
        buttons = [[
            ...
            InlineKeyboardButton('≪ Bᴀᴄᴋ', callback_data='helpsk'), 
            InlineKeyboardButton('⛺ Hᴏᴍᴇ', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons= [[
            InlineKeyboardButton('♥️ DONATION ♥️', callback_data='deploy')
            ],[
            InlineKeyboardButton('⛺ Hᴏᴍᴇ', callback_data='start'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            await message.reply_photo(photo=imdb.get('PICS'), caption=cap[:1024],
                                      reply_markup=InlineKeyboardMarkup(btn))
